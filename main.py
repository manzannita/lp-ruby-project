...
import semantico as ruby_semantico
...
class TabSemantico(tk.Frame):
    ...
    def _build(self):
        ...
        tk.Label(right, text="Errores semánticos", bg=BG_MAIN, fg=FG_MAIN,
                 font=FONT_UI).pack(anchor="w", pady=(0, 4))

        tree_frame = tk.Frame(right, bg=BG_MAIN)
        tree_frame.pack(fill="both", expand=True)

        cols = ("#", "Categoría", "Línea", "Columna", "Mensaje")
        self.tree = ttk.Treeview(tree_frame, columns=cols, show="headings",
                                 style="Dark.Treeview", selectmode="browse")
        for c, w, anchor in (("#", 40, "center"), ("Categoría", 150, "w"),
                             ("Línea", 56, "center"), ("Columna", 64, "center"),
                             ("Mensaje", 320, "w")):
            self.tree.heading(c, text=c)
            self.tree.column(c, width=w, anchor=anchor,
                             stretch=(c == "Mensaje"))
    ...
    def _analizar(self):
        ...
        self.status_cb("Analizando semántica...")
        for item in self.tree.get_children():
            self.tree.delete(item)
        self.update_idletasks()

        try:
            resultado = ruby_semantico.analizar(codigo)
            self._ultimo_resultado = resultado

            # Caso 1: hay errores sintácticos → no se puede analizar semántica.
            if resultado["errores_sintacticos"]:
                n = len(resultado["errores_sintacticos"])
                self.estado_var.set(
                    f"⚠  {n} error(es) de sintaxis — corrígelos antes del "
                    f"análisis semántico")
                self.estado_lbl.config(fg="#e5c07b")
                for i, err in enumerate(resultado["errores_sintacticos"]):
                    tag = "even" if i % 2 == 0 else "odd"
                    self.tree.insert("", "end", values=(
                        i + 1, "Sintaxis", err["linea"], err["columna"],
                        err["mensaje"]), tags=(tag,))
                self.status_cb(f"{n} error(es) de sintaxis")
                return

            # Caso 2: análisis semántico realizado.
            errores = resultado["errores"]
            if resultado["ok"]:
                self.estado_var.set("✔  Código semánticamente VÁLIDO")
                self.estado_lbl.config(fg="#5fd35f")
            else:
                self.estado_var.set(
                    f"✘  CON ERRORES — {len(errores)} error(es) semántico(s)")
                self.estado_lbl.config(fg=FG_ERROR)

            for i, err in enumerate(errores):
                tag = "even" if i % 2 == 0 else "odd"
                col = err["columna"] if err["columna"] else "-"
                self.tree.insert("", "end", values=(
                    i + 1, err["categoria"], err["linea"], col,
                    err["mensaje"]), tags=(tag,))

            self.status_cb(
                "Análisis semántico correcto" if resultado["ok"]
                else f"{len(errores)} error(es) semántico(s)")
        except Exception as e:
            messagebox.showerror("Error interno", str(e))
            self.status_cb("Error durante el análisis")
...
class App(tk.Tk):
    ...
    def _build(self):
        ...
        self.tab_semantico = TabSemantico(nb, self._set_status)
        nb.add(self.tab_semantico, text="  Semántico  ")
